...
| Field | Value |
|:------|:------|
| **Default Model** | Qwen 3.7 Max (`qwen3.7-max` via `maxplus`) |
| **MoA Preset** | `moa:ceo-moa` — ใช้เฉพาะงานยาก |
| **Mode A: Direct** | `ceo-model` (Qwen 3.7 Max) — routine, delegation, command |
| **Mode B: MoA** | `moa-ceo` (DeepSeek V4 Pro + Kimi K2.5 → Qwen 3.7 Max) — vision ใหม่, exception CRITICAL, architecture decision |
